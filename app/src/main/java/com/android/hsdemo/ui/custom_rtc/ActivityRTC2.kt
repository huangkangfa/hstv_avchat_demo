...
import com.android.hsdemo.databinding.ActivityRtc2Binding
...
import kotlinx.android.synthetic.main.activity_rtc2.*
import java.util.*
import kotlin.concurrent.fixedRateTimer

class ActivityRTC2 : BaseActivity<VMRTC2, ActivityRtc2Binding>(),
    View.OnClickListener {

    companion object {
        fun start(context: Context, userId: String, roomId: String, roomName: String) {
            val intent = Intent(context, ActivityRTC2().javaClass)
    ...
    override fun getLayoutId(): Int = R.layout.activity_rtc2
    ...
    val itemOfData = ListItem<ItemOfMemberInfo>(
        ...
        { holder, item ->
            ...
            val vv = holder.getView<TextureView>(R.id.mTXCloudVideoView)
            ...
            if (item._type == 0) {
                ...
                if (item._data.videoOpen == true) {
                    ...
                    mViewModel.myselfRenderVideoFrame.start(vv)
                } else {
                    XLog.i("【视频会议】尝试关闭小窗口本地预览视频 ${item._data.account.toString()}")
                    mViewModel.myselfRenderVideoFrame.stop()
                }
            } else {
                tv.text = item._data.nickName
                if (item._data.videoOpen == true) {
                    XLog.i("【视频会议】尝试开启小窗口远端用户视频 ${item._data.account.toString()}")
                    AVChatManager.getTRTCClient().setRemoteVideoRenderListener(item._data.account, TRTCCloudDef.TRTC_VIDEO_PIXEL_FORMAT_I420, TRTCCloudDef.TRTC_VIDEO_BUFFER_TYPE_BYTE_ARRAY,mViewModel.othersRenderVideoFrame[item._data.account])
                    mViewModel.othersRenderVideoFrame[item._data.account]?.start(vv)
                } else {
                    XLog.i("【视频会议】尝试关闭小窗口远端用户视频 ${item._data.account.toString()}")
                    mViewModel.othersRenderVideoFrame[item._data.account]?.stop()
                }
            }
    ...
    private fun initDefault() {
        ...
        mViewModel.initRecycleView(
            screenRight,
            LinearLayoutManager(this@ActivityRTC2, LinearLayoutManager.VERTICAL, false),
            itemOfData
        )
         ...
         * 参会人员变动
         */
        EventBus.with(EventKey.MEETING_USER_CHANGE, String::class.java).observe(this, Observer {
            dialogPeopleList.data = mViewModel.getPeopleList()
    ...
    private fun requestPermissions() {
        RxPermissions(this@ActivityRTC2).request(
            Manifest.permission.WRITE_EXTERNAL_STORAGE,
            Manifest.permission.CAMERA,
            Manifest.permission.RECORD_AUDIO,
            Manifest.permission.READ_EXTERNAL_STORAGE
        ).autoDispose(this@ActivityRTC2)
            .subscribe { granted ->
                if (granted) {
                    //权限允许的情况下
                    initStart()
    ...
    private fun initStart() {
        ...
        mViewModel.setLocalPreviewViewByFirst(vTRTCMain)
        startTimer()
    ...
    private fun updateMeetingDetail() {
        ...
        if (mViewModel.myself._data.videoOpen == true) {
            ...
            mViewModel.myselfRenderVideoFrame.start(mViewModel.getLocalPreviewView())
        } else {
    ...
    private fun initAnimotion() {
        animScreenTopGone =
            getAnimation(this@ActivityRTC2, R.anim.top_out, object : Animation.AnimationListener {
        ...
        animScreenRightGone =
            getAnimation(this@ActivityRTC2, R.anim.right_out, object : Animation.AnimationListener {
                override fun onAnimationStart(p0: Animation?) {
                }
        ...
        animScreenTopVisible =
            getAnimation(this@ActivityRTC2, R.anim.top_in, object : Animation.AnimationListener {
                override fun onAnimationStart(p0: Animation?) {
        ...
        animScreenRightVisible =
            getAnimation(this@ActivityRTC2, R.anim.right_in, object : Animation.AnimationListener {
                override fun onAnimationStart(p0: Animation?) {
        ...
        animScreenBottomVisible =
            getAnimation(this@ActivityRTC2, R.anim.bottom_in, object : Animation.AnimationListener {
                override fun onAnimationStart(p0: Animation?) {
